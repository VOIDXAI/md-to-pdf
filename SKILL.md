...
- `--performance-mode` — disable conservative Mermaid chunking and use maximum throughput

The script handles everything automatically:
1. Extracts all ` ```mermaid ` blocks and batch-renders them in conservative chunks by default (4 at a time) to avoid resource spikes
2. Reuses cached Mermaid SVGs across runs, so repeated exports of the same diagrams are much faster
3. If `--performance-mode` or `md_to_pdf.performance_mode: true` is set, uses aggressive full-batch Mermaid rendering for maximum speed
4. **Inlines SVG** directly into the HTML (vector quality — no rasterization, sharp at any zoom)
5. Each inline SVG gets a unique ID (`mmd-00`, `mmd-01`, ...) to avoid CSS conflicts
6. Injects `<a id="slug">` anchors before every heading (h1–h6) with hidden links to force Chromium to generate named destinations for all headings
7. Converts processed markdown → PDF via `md-to-pdf` npm package (Chromium-based)
8. Adds page numbers (footer: page / total)
9. **Generates precise PDF bookmarks** by reading Chromium's named destinations — all headings get exact page numbers (with character-position estimation as fallback)
10. Clears the PDF `/Title` metadata so Adobe Reader displays the OS filename
